...
class DroneController {
  ...
  async index(req, res) {
    const { page = 1, onlyTracking = false } = req.query;
    const drones = await droneService.findAll(page, onlyTracking);
  ...
  async store(req, res) {
    ...
    let drone = await droneService.findDroneByName(name);
    if (drone) {
      return res.status(400).send('Drone com este nome já está cadastrado');
    }

    if (temperature < -25 || temperature > 40) {
      return res
        .status(400)
        .send('Temperatura inválida. Deve estar entre -25 e 40');
    }
    if (humidity < 0 || humidity > 100) {
      return res
        .status(400)
        .send('Umidade inválida. Deve estar entre 0 e 100.');
    }
    if (latitude < -90 || latitude > 90) {
      return res
        .status(400)
        .send('Latitude inválida. Deve estar entre -90 e 90.');
    }
    if (longitude < -180 || longitude > 180) {
      return res
        .status(400)
        .send('Longitude inválida. Deve estar entre -180 e 180.');
    }
  ...
  async update(req, res) {
    ...
    drone = await droneService.findByIdAndUpdate(drone._id, req.body, {
      new: true,
    });
    return res.json(drone);
  }
